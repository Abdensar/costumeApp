...
try {
    $initialCounts = [
        'costumes' => DB::table('costumes')->count(),
        'categories' => DB::table('categories')->count(),
        'sizes' => DB::table('sizes')->count(),
        'costume_images' => DB::table('costume_images')->count(),
    ];
    $summary['initial_counts'] = $initialCounts;

    // Ensure admin exists (create if not) — will be rolled back
    ...
    // Read end counts (inside transaction)
    $endCounts = [
        'costumes' => DB::table('costumes')->count(),
        'categories' => DB::table('categories')->count(),
        'sizes' => DB::table('sizes')->count(),
    ...
    $summary['end_counts'] = $endCounts;

    // Rollback so DB is not modified
    DB::rollBack();

    // Read actual counts after rollback
    $actualCounts = [
        'costumes' => DB::table('costumes')->count(),
        'categories' => DB::table('categories')->count(),
        'sizes' => DB::table('sizes')->count(),
    ...
    $summary['actual_counts_after_rollback'] = $actualCounts;
